...
final profileRes=Provider.of<ProfileViewModal>(context);
    return Scaffold(
        appBar: AppBar(
        ...
        body: RefreshIndicator(
          onRefresh: () {
            return profileRes.dataFromProfileApi();
          },
          child: Center(
              child: profileRes.profileList==null
  ...
  Widget _profileData(ProfileViewModal profileRes) {
    return Expanded(
      child: profileRes.profileList?.results?.length == 0
          ? Center(
              child: Text(
              "No data found",
          ...
          : ListView.builder(
              itemCount: profileRes.profileList?.results?.length ?? 0,
              itemBuilder: (context, index) {
                final resData = profileRes.profileList!.results![index];
